        ...
        if Value == true or Value == false then
            if Value then
                if RageUI.CurrentMenu ~= nil then
                    ...
                    if Menu.UpdateInstructionalButtons then Menu:UpdateInstructionalButtons(Value) end
                    if Menu.UpdateCursorStyle then Menu:UpdateCursorStyle() end
...
function RageUI.CloseAll()
    if RageUI.CurrentMenu ~= nil then
        local menuToClose = RageUI.CurrentMenu
        local parent = menuToClose.Parent
        while parent ~= nil do
            parent.Index = 1
        ...
        menuToClose.Index = 1
        menuToClose.Pagination.Minimum = 1
        menuToClose.Pagination.Maximum = menuToClose.Pagination.Total
        menuToClose.Open = false
        RageUI.CurrentMenu = nil
        if menuToClose.Closed ~= nil then
            menuToClose.Closed()
        end
    end
    RageUI.Options = 0
...
function RageUI.Banner()
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu ~= nil and (CurrentMenu.Display.Header) then
...
function RageUI.Subtitle()
    local CurrentMenu = RageUI.CurrentMenu
    if CurrentMenu ~= nil then
        if CurrentMenu ~= nil and (CurrentMenu.Display.Subtitle) then
            RageUI.ItemsSafeZone(CurrentMenu)
...
function RageUI.Background()
    local CurrentMenu = RageUI.CurrentMenu;
    if CurrentMenu ~= nil then
        if CurrentMenu ~= nil and (CurrentMenu.Display.Background) then
            RageUI.ItemsSafeZone(CurrentMenu)
        ...
        if CurrentMenu ~= nil then
            ...
            if (CurrentMenu.Display.InstructionalButton) then
                if not CurrentMenu.InitScaleform then
                    if CurrentMenu.UpdateInstructionalButtons then CurrentMenu:UpdateInstructionalButtons(true) end
                ...
                if CurrentMenu.Controls.Back.Pressed and CurrentMenu.Closable then
                    ...
                    if CurrentMenu.Parent ~= nil then
                        if CurrentMenu.Parent ~= nil then
                            ...
                            if CurrentMenu.UpdateCursorStyle then CurrentMenu:UpdateCursorStyle() end
                        else
                            RageUI.NextMenu = nil
