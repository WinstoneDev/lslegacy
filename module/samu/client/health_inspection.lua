...
--  samu:hiPoll → samu:hiPollResult), écritures → events tokenisés
--  (samu:hiUseItem → samu:hiUseItemResult).
...
function SAMU.OpenHealthInspection(targetSrc)
    LSLegacy.Events.SendToServer('samu:hiOpen', { target = targetSrc })
end
...
RegisterNUICallback('hi:useItem', function(data, cb)
    if type(data) == 'table' and hiTarget then
        LSLegacy.Events.SendToServer('samu:hiUseItem', {
            target = hiTarget,
            part   = data.part,
            item   = data.item,
        })
    end
    cb('ok') -- la réponse arrive de façon asynchrone via samu:hiUseItemResult
end)

LSLegacy.Events.Register('samu:hiUseItemResult', function(payload)
...
LSLegacy.Events.Register('samu:hiPollResult', function(payload)
    if type(payload) ~= 'table' then return end
...
RegisterNUICallback('hi:poll', function(_, cb)
    ...
    LSLegacy.Events.SendToServer('samu:hiPoll', { reqId = reqId, target = hiTarget })
