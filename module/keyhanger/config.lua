-- =====================================================================
--  UTILITY KEYHANGER — Porte-clés mural physique & interactif
--  Module LSLegacy (Bastien MAGAN)
--
--  Permet d'installer des porte-clés muraux dans le monde : maisons,
--  appartements, garages, entreprises, factions, postes de police...
--  Les clés de véhicules y sont accrochées physiquement (props visibles
--  en direct par tous les joueurs), récupérables et partageables.
--
--  Fichier de configuration partagé (client + serveur).
-- =====================================================================
...
--  Comportement de l'item lorsqu'il est utilisé depuis l'inventaire.
-- ---------------------------------------------------------------------
C.Key = {
...
-- ---------------------------------------------------------------------
--  PLACEMENT (création de porte-clés en jeu)
-- ---------------------------------------------------------------------
C.Placement = {
...
-- ---------------------------------------------------------------------
--  STOCKAGE (DataStore)
--  Chaque support est adossé à un DataStore (comme un coffre de véhicule).
--  Les clés y sont déposées/récupérées via l'inventaire (et non un menu).
-- ---------------------------------------------------------------------
C.Storage = {
...
-- ---------------------------------------------------------------------
--  MODÈLES DE SUPPORT (planche murale)
--  Par défaut on utilise des props natifs GTA V (fonctionnent sans stream).
--  Pour un modèle custom : déposez le .ydr/.ytd dans module/keyhanger/stream/
--  puis ajoutez ici son nom (voir stream/README_PROPS.md).
-- ---------------------------------------------------------------------
--  ⚠️ Modèles VÉRIFIÉS existants dans GTA V de base (extraits du jeu).
C.Boards = {
    ["board_cork"] = {
        label = "Liège",
...
-- ---------------------------------------------------------------------
--  PROPS DE CLÉS
--  Pool de modèles de clés (natifs GTA V) ; chaque clé accrochée se voit
--  attribuer un prop pour donner de la variété visuelle. Le choix est
--  déterministe (basé sur la plaque) pour rester cohérent entre clients.
-- ---------------------------------------------------------------------
--  ⚠️ Modèles VÉRIFIÉS existants dans GTA V de base.
C.KeyProps = {
...
-- ---------------------------------------------------------------------
--  DISPOSITIONS DE SLOTS
--  Position de chaque clé RELATIVE au support, exprimée en :
--    x = décalage latéral (droite +)   en mètres
--    y = décalage profondeur (avant +) en mètres  (clés devant la planche)
--    z = décalage vertical (haut +)    en mètres
--  Réglez finement avec C.Debug = true (le gizmo affiche les index de slot).
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
--  TYPES D'ACCÈS (qui peut accrocher / récupérer des clés sur le support)
...
-- ---------------------------------------------------------------------
--  ANIMATIONS (accrocher / récupérer une clé)
-- ---------------------------------------------------------------------
C.Anim = {
