...
---Échange la position de deux items dans l'inventaire (ordre persisté en DB via player.inventory).
---@type function
---@param player table
---@param itemA table  { uniqueId?: number, name: string, label: string }
---@param itemB table  { uniqueId?: number, name: string, label: string }
---@return any
---@public
LSLegacy.Inventory.SwapItemsInInventory = function(player, itemA, itemB)
    if not player then return end
    ...
    for k, v in pairs(inventory) do
        if itemA.uniqueId ~= nil then
            if v.uniqueId == itemA.uniqueId then idxA = k end
        elseif v.uniqueId == nil and v.name == itemA.name and v.label == itemA.label then
            idxA = k
        end

        if itemB.uniqueId ~= nil then
            if v.uniqueId == itemB.uniqueId then idxB = k end
        elseif v.uniqueId == nil and v.name == itemB.name and v.label == itemB.label then
            idxB = k
        end
    end

    if not idxA or not idxB or idxA == idxB then return end
